...
let select = (str, type) =>
{
	switch (type)
	{
		case 'bankName':
			...
			if (bankName) bankName = bankName.match(/"\>.+?\<\/a\>/i) && bankName.match(/"\>.+?\<\/a\>/i)[0] || '';
			if (bankName && typeof bankName === 'string') bankName = bankName.slice(2, -4).trim().toLowerCase();
			...
			{
				if (bankName.indexOf(bankNamesCompare[i]) >= 0) return bankNames[i];
			}
			return bankName;
